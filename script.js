...
const SEEK_STALL_MS = 250; // if a seek hasn't resolved by this point, stop waiting on it and issue a fresh one
...
function applyScrub() {
  ...
  //
  // But `seeking` isn't guaranteed to ever clear - on some hosts (seen on
  // GitHub Pages, not the local dev server) a seek can just hang, and
  // waiting on `seeking` forever then freezes the video at whatever frame
  // it was stuck on for the rest of the page's life. SEEK_STALL_MS bounds
  // how long we'll defer to an in-flight seek before overriding it with a
  // fresh one anyway, so a stuck seek degrades to "occasionally skips a
  // beat" instead of "permanently frozen".
  const seekIsStale = Date.now() - lastSeekIssuedAt > SEEK_STALL_MS;
  if (!heroVideo.seeking || seekIsStale) {
    heroVideo.currentTime = videoProgress * heroVideo.duration;
    lastSeekIssuedAt = Date.now();
  }
